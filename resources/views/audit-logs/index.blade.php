            ...
            <div>
                ...
                <select name="action"
                    ...
                    @foreach (['created' => 'Tambah', 'updated' => 'Ubah', 'deleted' => 'Hapus'] as $value => $label)
                        <option value="{{ $value }}" {{ $action == $value ? 'selected' : '' }}>{{ $label }}
                        </option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col">
        <div class="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <form method="GET" action="{{ route('audit-logs') }}"
                class="flex items-center justify-between sm:justify-end gap-3 text-sm text-slate-500">
                                    ...
                                    <ul class="space-y-1">
                                        @foreach ($log->new_values as $field => $newValue)
                                            <li>
                                                ...
                                                <span
                                                    class="text-rose-500 line-through">{{ $log->old_values[$field] ?? '-' }}</span>
                                                &rarr;
            ...
            @forelse ($auditLogs as $log)
                <div class="p-4 transition-colors hover:bg-slate-50">
                    <div class="flex items-start justify-between gap-2 mb-2">
                        <div>
                            <p class="text-xs text-slate-400">{{ $log->created_at->format('d/m/Y H:i:s') }}</p>
                        ...
                        @endphp
                        <span
                            class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium {{ $badgeClass }}">
                            {{ $badgeLabel }}
                        </span>
                    </div>
                    <div class="grid grid-cols-2 gap-y-1 text-xs text-slate-500">
                        <span>Pengguna</span>
                        <span class="text-right text-slate-700 font-medium">{{ $log->user?->name ?? 'Sistem' }}</span>
                        <span>IP Address</span>
                        <span class="text-right text-slate-700 font-medium">{{ $log->ip_address ?? '-' }}</span>
                    </div>
                    @if (($log->action === 'updated' && $log->new_values) || $log->action === 'created' || $log->action === 'deleted')
                        <div class="mt-2 rounded-lg bg-slate-50 p-2.5 text-xs">
                            <p class="mb-1 font-medium text-slate-400">Detail Perubahan:</p>
                            @if ($log->action === 'updated' && $log->new_values)
                                <ul class="space-y-1">
                                    @foreach ($log->new_values as $field => $newValue)
                            ...
                            @elseif ($log->action === 'created')
                                <span class="text-slate-400">Data baru dibuat</span>
                            @elseif ($log->action === 'deleted')
                                <span class="text-slate-400">Data dihapus</span>
                            @endif
                        </div>
                    @endif
                </div>
